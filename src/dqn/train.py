"""DQN training for poker games using the existing Game abstraction.

State = game.infoset_to_features(infoset).  Actions = 0..num_actions-1.
"""
import random
import os
import numpy as np
...
def train_dqn(iterations, game_name="kuhn", log_prefix="dqn"):
    game = get_game(game_name)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    agent = DQNAgent(game, device)

    print(f"DQN  |  game: {game.name}  |  episodes: {iterations:,}  |  device: {device}")
